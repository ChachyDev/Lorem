...
class Launcher(launcher: Launcher.() -> Unit) {
    ...
    suspend fun begin() {
        ...
        if (authType == AuthType.Mojang && password?.isEmpty() == true || authType == AuthType.Mojang && username.isEmpty()) error(
            "Please specifiy a username and password"
        )
        ...
        val manifest = if (!isCustomClient) ManifestTask(runDir, jvmArgs.toMutableList()).execute(version) else CustomManifestTask(runDir, jvmArgs.toMutableList()).execute(version)
        if (!isCustomClient) {
